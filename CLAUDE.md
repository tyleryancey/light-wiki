...
**Network contract:** base `https://en.wikipedia.org/w/api.php`, `formatversion=2`, GET only — (1) `action=query&list=search&srsearch=<q>&srlimit=20`; (2) `action=query&prop=pageprops&ppprop=disambiguation&redirects=1&titles=<t>`; (3) `action=parse&prop=text&redirects=1&page=<t>`. Plus image GETs to `upload.wikimedia.org` only. No other host, ever; `WikiHosts.assertAllowed` enforces the two-host allowlist at both client seams (`KtorWikiApi.get`, `Images.load`) in every build type — there is no `BuildConfig`/`DEBUG` guard anywhere in `tool/src/main`.
...
- **Physical LP3 pass complete** (TLP301 `LP3LHMA531900321`, sideloaded unvetted, committed `serverPackage = "com.lightos"` — no toml flip, as hardware requires). Driven end-to-end on the real panel: search idle → full-screen editor (LP3 keyboard UI) → live "mercury" results → disambiguation chooser → article round-trips (planet + element via back stack) → infobox card (entity-decoded −/° glyphs correct) → **grayscale image rendered on the physical AMOLED** (Great Wave plate under an h2 — the M6 spike claim now proven on hardware) → A/A two steps up with position held → airplane-mode error + RETRY with friendly copy → airplane restored → killstart lands on Search (by design). The ringtone-precedent claim held exactly: no RPCs, so nothing NoPermission-gated; everything worked unvetted.
- Screenshot harvest in `tool/screenshots/` (ringtone M4 convention): 10 shots from the physical device covering the main flow (idle, editor, results, chooser, two articles, image, A/A, offline error, relaunch). Every 03 §2 state was *demonstrated* during the drive; the loading/empty transients of each screen are not individually screenshotted.
- Live-fetch smoke: the whole hardware pass ran against live Wikipedia (search, pageprops, parse, images) — no HTML-shape drift vs the 2026-07-28 fixtures.
- Scroll of the largest article was smooth while driving; cold launch ~4 s including the SDK interstitial (subjective from adb driving — no instrumented perf run).
- `workspace-health` fleet sweep run (2026-07-28): light-wiki clean (no unpushed commits, toml correct; sync workflow simply hadn't had its first scheduled run; 2 behind upstream = the toggle-icons commits, resolved by the first sync). Sibling-repo findings (5 blocked sync PRs, 3 red workflows, legacy light-tools drift) reported to Tyler — out of this repo's scope.
- Display-math research (Tyler question): the PNG variant endpoint **exists and serves image/png** (`wikimedia.org/api/rest_v1/media/math/render/png/<hash>`, verified live 2026-07-29), so option B (third scoped Wikimedia host + inversion ColorFilter) is proven feasible; recommendation on record: ship v1 with math dropped (option A), B as first backlog item. **Update (2026-07-31):** a 42-hash sample of this repo's own fixture hashes found 16 returning SVG bytes under that same `image/png` header — the content-type alone did not establish feasibility; detail in `rendering-exclusions.md` §10.5.
- M7 frozen-table review (fresh-context, full functional audit, both directions): contract holds — every In-row implemented (display-math deviation properly recorded), nothing from Never/Out present, single-key persistence confirmed, no RPCs, defense mechanics all green. Findings, all fixed: (1) IMPORTANT — dead `ksp(room-compiler)` scaffold residue in `tool/build.gradle.kts`, the one thing beyond the table (a free "why does a no-storage tool declare Room?" vetting question) — removed; (2) DataStore reads/writes now degrade instead of crashing on unreadable prefs (`.catch { emptyPreferences() }` / try-catch); (3) screenshot-coverage sentence reworded to claim only what the shots show; (4) two stale comments (BlockRenderer "M6 lands later", SnippetText "Jsoup lands in M3") corrected to shipped reality.
...
- M6 review found 7 (1 important): (1) the downsample loop's invariant was inverted (decoded width landed in [1080, 2160)) — fixed to a true ≤ bound with pure-JVM `sampleSize` tests (RED first), plus a height bound (2480 px) so any single bitmap's bytes are capped; (2) `display:none` match now case/whitespace-insensitive (test first); (3) `width="0"` no longer crashes `aspectRatio` (guard symmetry); (4) caption renders only when the image is Ready — no transient orphan caption during slow failures; (7) all failure paths now log; exclusions §10.8 re-cited to the actual enforcement points. Accepted for v1 with backlog notes: (5) wide-table column drift beyond 6 em cells (§10.7 scopes v1 to a simplified grid — judge on a wide fixture in M7), (6) evicted images re-fetch on scroll-back (no disk cache; in-memory LRU satisfies the plan's wording).
